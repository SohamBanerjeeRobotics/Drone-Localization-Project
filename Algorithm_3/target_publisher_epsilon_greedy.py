...
def main(args = None):
    ...
    try:
        ...
        if len(X_array) > 0:
            ...
            plt.xlabel("X --->")
            plt.ylabel("Y --->")
            plt.title("Drone path during Fire Localization Epsilon Greedy - Algorithm 3")
            plt.grid(True)
            plt.savefig("/home/soham-banerjee/Algorithm/Algorithm_3/Trajectory_Epsilon_Greedy.png")
            plt.show()
        else:
