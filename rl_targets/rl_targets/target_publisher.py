...
def temperature_distribution_plot(x_pos_drone, y_pos_drone, T0 = 20.0, SX = 13.5, SY = 13.0, ST = 100.0, Z = 1.0, DTMax = 200.0, LT = 2.0, KH = 0.1):
    ...
    local_source_x = 9.0
    local_source_y = 8.66
    local_source_temp = 100
    local_source_temp_drop_rate = 0.5

    r_drone_local_source = np.sqrt(((x_pos_drone - local_source_x) ** 2) + ((y_pos_drone - local_source_y) ** 2))
    T_additional = (-local_source_temp) * np.exp((-local_source_temp_drop_rate) * r_drone_local_source)

    return T_drone + T_additional
...
def main(args = None):
    ...
    try:
        ...
        plt.scatter(3850, 1200, color = "green")
        if len(X_array) > 0:
            ## Saving array for metrics.
            np.savez_compressed("/home/soham-banerjee/Algorithm/Algorithm_1/Greedy/Case_4e_Final/Data_Case_4e.npz", x_data = np.array(X_array), y_data = np.array(Y_array), x_actual_source = 13.5, y_actual_source = 13.0, x_detected_source = fire_temperature_loc_x, y_detected_source = fire_temperature_loc_y)
            plt.plot((2500 + (np.array(X_array) * 100)), (2500 - (np.array(Y_array) * 100)), 'b.-')
            
            ## To add custom tick labels.
            x_tick_positions = [0, 1250, 2500, 3750, 5000]  # Pixel positions
            x_tick_labels = ['-25', '-12.5', '0', '12.5', '25']  # Real values
            
            y_tick_positions = [0, 1250, 2500, 3750, 5000]  # Pixel positions
            y_tick_labels = ['25', '12.5', '0', '-12.5', '-25']  # Real values
            ...
            plt.xlabel("X --->")
            plt.ylabel("Y --->")
            plt.title("Drone path during Fire Localization Greedy - Algorithm-1 (Case 4e)")
            plt.grid(True)
            plt.savefig("/home/soham-banerjee/Algorithm/Algorithm_1/Greedy/Case_4e_Final/Trajectory_Case_4e.png")
            plt.show()
        else:
