...
Epsilon = 0.4 # for epsilon greedy
    ...
    def greedy(self):
        if self.pub_step != 0:
            ...
            if distance_remaining <= 0.02:
                ...
                if self.pub_step >= 72:
                    if self.pub_step == 72:
                        print(f"moving in circle completed. all gradients info have been succesfully collected.")
                        ...
                        print(f"moving in a straight line in the epsilon-greedy grad direction.")
...
def main(args = None):
    ...
    try:
        ...
        if len(X_array) > 0:
            ...
            np.savez_compressed("/home/soham-banerjee/Algorithm/Algorithm_1/EpsilonGreedy/Case_1/Data_Case1.npz", x_data = np.array(X_array), y_data = np.array(Y_array), x_actual_source = 13.5, y_actual_source = 13.0, x_detected_source = fire_temperature_loc_x, y_detected_source = fire_temperature_loc_y)
            ...
            plt.xlabel("X --->")
            plt.ylabel("Y --->")
            plt.title("Drone path during Fire Localization Epsilon Greedy - Algorithm-1 (Case 1)")
            plt.grid(True)
            plt.savefig("/home/soham-banerjee/Algorithm/Algorithm_1/EpsilonGreedy/Case_1/Trajectory_Case1.png")
            plt.show()
        else:
